...
class ValidationMixin:
    ...
    SLUG = r"^[a-zA-Z0-9\-]+$"
    NUMERIC = r"^\d+$"
    EMAIL = r"^(?!.*\.{2})[a-zA-Z0-9._%\-+]+\@(?!.*\.{2})[a-zA-Z0-9._%\-]+\.[a-zA-Z]{2,}$"
    USERNAME = r"^[a-z0-9._]{3,}$"
    DATE = r"^(19|20)\d\d[- /.]([1-9]|0[1-9]|1[012])[- /.]([1-9]|0[1-9]|[12][0-9]|3[01])$"
    TIME = r"^([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$"
    DATETIME = r"^(19|20)\d\d[- /.]([1-9]|0[1-9]|1[012])[- /.]([1-9]|0[1-9]|[12][0-9]|3[01])\s(0|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$"
    ...
    def valid(self, arg_name, validator = None, help = None, name = None, required = True, **kwargs):
        ...
        if self.request.headers.get("Content-Type") and\
                "application/json" in self.request.headers.get("Content-Type").lower():
        ...
        # doesn't match regex format ?
            ...
            if re.match(validator, str(value), re.IGNORECASE) is None:
                self._help(arg_name, help, name, ValidationMixin.MSG_DEFAULT)
